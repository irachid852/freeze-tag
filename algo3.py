# -*- coding: utf-8 -*-
"""
Created on Wed Jun  4 10:40:33 2025
...
def initial(n,lsc=None):
    if lsc == None:
        ...
        print("bizarre")
    else : lsr = [Robot([0,0],True)]+ [Robot(lsc[i-1][:], False) for i in range(1,1+n)] 
    ...
    #aa.ft.draw_disc()
    dicc = {'couronne 1': "ro", 'couronne 2': "bo"}
    #print(minval2)
    #print(minval2.coord)
    ...
    for e in lsr:
        ...
        if 0 <= (t-minval.coord[0])%(2*np.pi) < np.pi:
            dic['couronne 1'].append(e)
            e.groupe = 'couronne 1'
        elif np.pi <= (t-minval.coord[0])%(2*np.pi) < 2*np.pi:
    ...
    for r in lsr:
        x,y = pol2car(r.coord)
        if r not in [lsr[0],minval]:
            
            plt.plot(x,y,dicc[r.groupe])
        else:
            plt.plot(x,y,"mo")
    plt.show()
    return lsr, minval, dic

def recherche_couronne(d, couronne, nom,minval,lsr):
    angleinf = ((int(nom[-1])-1)*np.pi)%(2*np.pi)
    anglesup = ((int(nom[-1]))*np.pi)%(2*np.pi)
    ...
    d.borne_rayon_inf = minval.coord[1]
    d.borne_rayon_sup = 1
    if d in couronne:
        couronne.remove(d)
    ...
    while not file.est_vide() and couronne:
        ...
        for e in couronne:
            ...
            g = int(nom[-1])
            if g == 1: diff = abs(t-((int(nom[-1])-1)*np.pi))
            else: diff = abs(t-((int(nom[-1]))*np.pi))
            if diff < min_diff and e.coord[1]!=0 and e.coord not in [minval.coord] and e!=r1:
                min_diff = diff
                mincour = e
        if mincour:
            elt.aller(mincour)
...
def recherche(n,lsc=None):
    ...
    lsr, minval, dic = initial(n,lsc)
    lsd = [minval,lsr[0]]
    #print("liste ", [pol2car(r.coord) for r in lsd])
    v = recherche_couronne(lsr[0],dic['couronne 1'],'couronne 1',minval,lsr)
    #print("couronne 1",pol2car(v.coord))
    
    lsd.remove(v)
    k = recherche_couronne(minval,dic['couronne 2'],'couronne 2',minval,lsr)
...
ls = [[rd.uniform(0,2*np.pi), np.sqrt(rd.uniform(0.1,1))] for i in range(500)]
lsr = recherche(500,ls)
